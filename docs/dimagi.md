...
Dimagi maintains several production CommCare Data Pipeline environments by making use of a submodule through which the different 
...
This requires access to the [`commcare-sync-inventories` repository on Github](https://github.com/dimagi/commcare-sync-inventories/).
...
[`commcare-sync-inventories` private repository](https://github.com/dimagi/commcare-sync-inventories/),
and then committed to the main repository by updating the submodule reference.

### Using the commcare-sync-inventories submodule in production
...
After adding a deploy key to the `commcare-sync-inventories` repository you should be able to 
