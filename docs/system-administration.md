...
CommCare Sync deployments use an
...
instance of CommCare HQ, you will need to add the server parameters.

1. Log into the CommCare Sync server using the superuser account you
   created above.
...
To activate the virtual environments for CommCare Sync and Superset, run
...
The CommCare Sync virtual environment is in
...
database server, or by different servers. Use an appropriate database
client to access them. [DBeaver](https://dbeaver.io/) for SQL Server
and PostgreSQL, and [pgAdmin](https://www.pgadmin.org/) for PostgreSQL
are popular choices.
...
Backups for the CommCare Sync database can also be done, but it is not
required as the data is already present in CommCare HQ. If you want to
add the CommCare Sync database as well, you may update
`postgresql_backup_db_list` to 'superset commcare_sync'.

To deploy the changes, either do a full stack deploy, or as a one-off
using:
